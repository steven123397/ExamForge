...
- Next.js 多角色工作台原型：真实会话登录、运行排考、草稿矩阵、建议调整、锁定重排、结果分析和教师/学生发布查询。
- Drizzle/PostgreSQL 数据层：schema、迁移、seed、关联表和持久化仓储。
- Docker Compose 演示栈：PostgreSQL、迁移、seed、FastAPI scheduler、API 和 Web 一键编排。
...
- 快速门禁：所有 `push`、PR 和手动触发均运行仓库治理检查、OpenAPI 漂移检查、类型检查、API 测试、scheduler 测试和生产构建。
...
该命令按 PostgreSQL 与 scheduler 健康检查、迁移、seed、API 就绪检查和 Web 健康检查的顺序启动服务。scheduler 使用独立的 Python/uv 镜像和非 root 用户；API 镜像只包含 Node.js 运行时，通过内部 HTTP 调用 scheduler，不包含 Python、uv 或算法源码。
...
烟测先直连 scheduler 验证健康、可行求解和业务不可行结果，再使用 `.env` 中的排考员密码建立真实服务端会话，并验证 Web/API、PostgreSQL readiness、独立 HTTP 调度、零硬冲突结果，以及 API 重启后的运行记录和会话持久化。
...
- `POSTGRES_PORT`、`SCHEDULER_PORT`、`API_PORT` 和 `WEB_PORT` 只覆盖宿主机映射端口，不改变容器内端口。
