...
项目已完成正式需求分析、总体设计、第一版实现内容设计、可行性分析、第二版第一阶段、第二版方案草稿工作台第二阶段和第三阶段。第一版 `apps/scheduler/` Python 排考算法原型的核心模块已合并到 `main`，包括数据模型、测试数据生成、预检、冲突解释、硬约束求解器、软约束评分和报告整理。第二版已形成“自动排考 → 草稿调整 → 硬约束校验 → 对比确认 → 发布/废弃 → 已发布查询”的人机协同排考闭环，并补齐局部调整建议、矩阵拖拽和浏览器级交互验收。
...
- [ ] 规划下一阶段：权限审计、报告导出、数据库迁移治理或课程报告终稿整理。
