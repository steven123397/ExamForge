...
- 快速与静态门禁：`npm run test:ci` 为 `9 passed`；发布专项当前为 `19 passed`，覆盖 Docker Engine builder、有界 apt 更新、逐镜像构建/推送边界和远程 digest 校验，部署类 Node.js 合同总计 `38 passed`；`npm run check:scheduler-openapi`、`npm run build`、`docker compose config --quiet` 的既有基线通过，CR-028 本地修复后 `npm run check:ci`、仓库级 `npm run typecheck`、Bash 语法和 `git diff --check` 重新通过。
