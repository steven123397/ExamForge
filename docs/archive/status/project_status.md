...
> **归档声明（2026-08-17）**：本文是迁移前的状态快照。正文中的分支、基线、下一步和“未合并”描述只代表记录时点；当前进度、验收和依赖以 GitHub Issues 为准，长期决策以 `docs/adr/` 为准。

## 1. 当前结论

- 第六版第一阶段“参与者数据诚实性与合同”已在本地命名工作树 `v6/participant-contracts` 完成实现、真实 PostgreSQL 验证和计划归档；本阶段只保留为独立分支成果，不合并 `main`、打标签、发布镜像或写入服务器。
- 第六版第二阶段已在 `v6/participant-contracts` 启动并完成任务 1、任务 2：shared、Python scheduler 与 Scheduler OpenAPI 已冻结报名重叠边和 participant 诊断合同；CP-SAT 在 `enrollments` 模式按重叠边阻止同槽，`groups_only` 与 v1/v2 snapshot 保持兼容。任务 3 至任务 6 尚未开始，`schedule-input-builder` 仍稳定拒绝 application 层的 enrollment 输入。
...
- CR-002、CR-028、CR-029、CR-030、CR-031、CR-032、CR-033、CR-034 与 CR-035 均已关闭。2026-07-17 的第五版双轨独立全量审查新增 CR-029 至 CR-036；2026-07-23 已完成角色授权、创建顺序、发布并发、登录失败防护、受控密码轮换、匿名公开 DTO 裁剪和有界草稿建议整改，当前问题明细为 P3 2 项，共 2 项、没有 P0/P1/P2。`v5.0.1` 保留为可验证历史制品，但其 `find-my-way@9.6.0` 已受高危通告影响，不得作为公开部署输入；详细审查问题只维护在 `docs/archive/status/code_review_status.md`。
- `v5.0.3` 的失败发布仅保留为历史诊断。`v5.0.5`（`f124861`）的正式发布工作流 `30091171562` 已成功，release manifest 在本地下载和服务器 `current` 目录均通过提交及附件校验；服务器当前按该不可变 manifest 运行，`v5.0.4`（`47e7af4`）保留为 `previous`。正式域名已完成 HTTPS 切流、首份备份恢复和主机级定时运维基线，用户也已完成浏览器人工登录并确认页面可用。`v5.0.5` 已将官方 online smoke 改为只接受独立、权限受控的当前凭据文件，不再读取首次建户变量或把生产 `.env` 当 shell 文件；`v5.0.6` 进一步避免 Node dotenv 对密码特殊字符的二次解释。服务器尚未设置独立凭据文件，因此未继续尝试登录；自动正式域名四角色 Playwright 和有限开放验收均未执行，按用户决定留待后续获准的维护窗口。
...
- `v5.0.5` 的运维补丁让 `run-online-smoke.sh` 强制接收权限为 `600` 的独立凭据文件，只解析四个允许的 `ONLINE_*_PASSWORD` 当前密码，不继承调用方同名变量，也不 `source` 或复制生产 `.env`。该文件不纳入仓库；服务器未配置它时脚本应在登录前失败，不能退回或猜测首次 bootstrap 密码。
...
### 2.12 v5.0.5 正式部署、v5.0.6 本地收尾与稳定性观察

- `f12486160dcacc09c00f29b6ed63110f1f3378a9` 的 `v5.0.5` 由正式工作流 `30091171562` 生成；四个应用镜像、SBOM、Trivy HIGH/CRITICAL 扫描、TCR digest、release manifest 和 artifact `8595999468` 均通过。发布前后的本地合同为 `npm run test:deploy` `52 passed`、`npm run test:ci` `9 passed`、`npm run test:release` `20 passed`、`npm audit --audit-level=moderate` 零漏洞、`npm run typecheck`、`npm run test:scheduler`（`97 passed`）和 `git diff --check`。
...
- 第六版第一阶段成果当前只存在于本地命名工作树，已整理为独立分支提交但尚未形成 release 输入。本阶段不把该工作树当作服务器部署来源；后续第六版发布须另行选择源码提交并重新走发布链。第二阶段也继续使用该分支，尚未形成 release 输入。
...
1. 第六版第二阶段按 `docs/archive/plan/第六版第二阶段计划.md` 待用户确认后启动；实现、测试和提交继续在 `v6/participant-contracts`，不自动创建 PR 或触发发布链。
2. 在第二阶段计划确认前，不把该分支用于 Runner、TCR、服务器部署或正式域名验证；第五版线上运维跟进仍按原边界处理。
