...
CR-028 修复提交后的工作流 `29482599323` 在首个 API 构建中连续 17 分钟没有新增字节、日志或子阶段证据，因而主动取消。逐镜像构建边界与 systemd 稳定路径随后以 `11ab909` 提交并推送；工作流 `29557441559` 的 GitHub 质量 job 成功，但 self-hosted job 在 `docker/setup-buildx-action` 的远端 BuildKit bootstrap 阶段连续 8 分钟无进展后取消，尚未开始任何业务镜像构建。独立 60 秒探针证明即使本机已有完整 `moby/buildkit` 镜像，`docker-container` 驱动仍会强制执行远端 pull；该时点本地修复因此移除该 Action 和 bootstrap 脚本，改为显式使用已验证的 Docker Engine `default` builder。发布合同为 `18 passed`，完整部署合同为 `37 passed`；当时 CR-028 尚无新 release 证据，后续结论见下文完成记录。

Engine builder 修复以 `a8ae2f2` 提交并推送后，工作流 `29558412245` 已证明质量 job、Engine builder 校验、审计下载和精确 SHA 校验通过；API 与 scheduler 均在首次尝试构建并通过职责探针。Web 第一次在下载 `sed` 安全更新时遇到代理 `502`，第二次在 Debian `InRelease` 遇到同类错误，故有界构建步骤失败；SBOM、Trivy、TCR 登录、推送和 manifest 全部跳过，失败诊断 artifact 与清理成功。对照探针显示相同容器经清华 Debian 镜像下载 9.3 MB 索引约 4 秒，而官方 CDN 连续 120 秒超时；本地 helper 随后把镜像选择、单次 120 秒和最多 3 次重试纳入日志合同。API、scheduler、Web、Worker 均在首次尝试完成真实 linux/amd64 构建和职责探针，apt 更新分别输出成功状态；该时点发布专项为 `19 passed`，完整部署合同为 `38 passed`，正式运行结论见下文 `a507993` 完成记录。

apt 修复以 `b9a2274` 提交并推送后，工作流 `29560455791` 的质量门禁、四镜像首次构建/探针、SBOM、Trivy HIGH/CRITICAL 门禁、TCR 登录、四次推送、远程 digest 读取和 manifest 校验均成功；正式 bundle 在 `CreateArtifact` 阶段因一次 `ECONNRESET` 上传失败，随后失败诊断 artifact 上传成功。由于正式 release artifact 缺失，本轮四个 TCR tag 与已校验 manifest 仍不得部署。该时点本地 workflow 已为正式 artifact 增加 5 秒后最多 1 次同名覆盖重试；发布专项和完整部署合同数量不变，后续完整运行结论见下文 `a507993` 记录。

artifact 重试修复以 `a507993` 提交并推送后，工作流 `29561565291` 于 2026-07-17 完整成功。GitHub 托管质量 job、本地 Engine builder 校验、四镜像首次构建与职责探针、SBOM、Trivy HIGH/CRITICAL、TCR 登录、四次推送、远程 digest 和 release manifest 校验全部通过；主 artifact 第一次上传即成功，因此等待与覆盖重试按条件跳过。正式 artifact 为 `examforge-release-a507993d9f9b0f2bdc879d421484ac3a28f67d74`，ID `8399608869`、压缩大小 `1691753` bytes、GitHub artifact digest `sha256:a237a6cdba26f73f4baf11cab38eec8c9403fb47a4ef8c6551e3dd9d86267025`。下载后的完整附件再次通过预期提交和文件校验，正式镜像 digest 为 API `sha256:d782067f...a6e1`、scheduler `sha256:3951d4bb...bc53`、Web `sha256:856bc984...22d5`、Worker `sha256:e57dcb5a...8afd`。

北京服务器使用独立 `ubuntu` 凭据按新旧两份 release manifest 拉取固定 digest，不访问 GitHub、不构建源码；`a507993` 仅回环部署完成迁移、职责健康、故障恢复、SSE `Last-Event-ID` 重连、备份恢复、真实回滚和重新部署后再次停止。四个应用容器中的 OCI revision 与当前 manifest 一致，`current` 保留 `a507993`、`previous` 保留 `f769725`；服务器没有使用本地 image ID 或部分 tag 作为发布依据。Runner 未持有服务器 SSH 私钥或生产 secrets，GitHub workflow 也未自动连接服务器；发布结束后前台 Runner 正常退出并保持离线。第六阶段任务 3 和 Runner 发布边界已经完成验证，备案、nginx/HTTPS 和正式域名 E2E 属于部署侧后续，不改变本设计的职责隔离结论。
